...
###load the scan1Results.RData since i ran it on server
load("scan1Results.RData")
...
peaks_full = find_peaks(out_full, map, threshold=threshold_full, drop=1.5, peakdrop = 1.5) 
peaks_gxe = find_peaks(gxe, map, threshold=threshold_gxe, drop=1.5, peakdrop =2) 
...
pdf('../3-Figures/1-QTLs on Genetic Map.pdf', width = 9,height = 7, onefile = T)

cross = read.cross("csv",file="../0-RawData/Brachypodium_RQTL data.csv")
cols  =  RColorBrewer::brewer.pal(n=as.numeric(length(unique(flank_marker_full$lodcolumn))), name = "Dark2") ###add your colors depending on how many traits you have
...
# cols  = c("red", RColorBrewer::brewer.pal(n=11,name = "BrBG"),"black")
...
pdf('../3-Figures/2-LOD Profiles for Different Traits with Different Models.pdf',width = 10,height = 7)
par(mfrow=c(4,1),oma = c(5,4,0,0) + 0.1,
    mar = c(0,0,0.6,1) + 0.1)
...
pdf('../3-Figures/2-LOD Profiles for All Traits with Full Models and GxE model.pdf',width = 10,height = 7 )
par(mfrow=c(2,1),oma = c(5,4,4,0) + 0.1,
    mar = c(0,0,0,1) + 0.1)
cols  = c("red", RColorBrewer::brewer.pal(n=11,name = "BrBG"),"black") ###pick your color..
...
mtext("LOD Score", side = 2, outer = T, line = 1.5)
legend(x=200,y=-0.5, legend=colnames(out_full),cex = 1,
       lwd=2, col = cols, bty="n", xpd=NA,  ncol = 4)
dev.off()
####END
